...
class CollectionsProg
{
    static void Main()
    {
        ...
        for (int j = 0; j < StudentsId.Count; j++)
        {
            Console.WriteLine($"Student {j + 1} id: {StudentsId[j]}");

            foreach (var st in stddetail)
            {


                Console.WriteLine($"Subject {j + 1} name: {st.Key}, Marks:{st.Value}");
                j = j + 1;
        ...
        foreach (var mar in avgmarks)
                {
                    Console.WriteLine($"Average:{mar}");
                }
        }
